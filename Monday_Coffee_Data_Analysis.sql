...
select ci.city_name,
		...
		round((sum(s.total) / count(distinct s.customer_id))::numeric, 2) as per_customer
...
with city_table as
(
select city_name, round((population * 0.25)/1000000) as coffee_consumers
from city
),
...
select 
		customers_table.city_name,city_table.coffee_consumers as coffee_cons_In_million,customers_table.unique_cx
from city_table
...
(select
		city_name,
		estimated_rent,
		round((population * 0.25)/1000000,3) as estimated_coffee_cunsumer_in_million
from city)
...
select 
	...
	ct.total_cx,
	estimated_coffee_cunsumer_in_million,
	ct.per_customer,
	round(
	cr.estimated_rent :: numeric/ct.total_cx :: numeric,2) as avg_rent_per_cx
from city_rent as cr 
